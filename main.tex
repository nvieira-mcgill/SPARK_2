...
%     \item Our priors for our multi-component fits are given in Table~\ref{tab:priors-multi}. These are identical at all epochs. The priors for the two components in a given fit are also identical, except that the ``red'' component has $Y_{e,1} \in [0.01, 0.45]$ while the ``blue'' has $Y_{e,2} \in [0.05, 0.50]$. We use the terms red and blue loosely to simplify our discussion but note that the priors do not enforce $Y_{e,1} < Y_{e,2}$. The priors enforce $v_{\mathrm{inner,1}} < v_{\mathrm{outer,1}}$ and $v_{\mathrm{inner,2}} < v_{\mathrm{outer,2}}$. We also ensure some overlap between the two components, \ie, $v_{\mathrm{outer,1}} > v_{\mathrm{inner,2}}$ or $v_{\mathrm{outer,2}} > v_{\mathrm{inner,1}}$. \redbf{Finally, we also enforce that there is some non-overlap between the two components, \ie, one component does not completely envelop another.} To allow for even sampling of parameter space for the $m_0 = 1500$ points in the base training set with this complex conditional prior, we use constrained Latin Hypercube Sampling (cLHS; \citealt{petelet09}).
...
\begin{figure*}[!ht]
    ...
    \figcaption{\textbf{Compilation of all best-fit models from this work: 1.4, 2.4, and 3.4 days post-merger, for both single- and multi-component models.} The single-component models are favoured at 1.4 and 2.4 days psot-merger. At 3.4 days, we require a multi-component ejecta to adequately fit the spectrum of AT2017gfo. The abundance patterns corresponding to these preferred models are included in Figure~\ref{fig:abunds_time_evolution}.}\label{fig:bestfits}
\end{figure*}
...
\begin{figure*}[!ht]
    ...
    \figcaption{\textbf{Best-fit abundance patterns at 1.4, 2.4, and 3.4 days.} The abundances at 1.4 and 2.4 days are taken from the preferred single-component fits, while the abundance at 3.4 days is taken from the multi-component fit. The overall abundance of the multi-component ejecta is the mass-weighted sum over all 10 shells in the stratified ejecta. Uncertainty bands are obtained by taking additional samples from the the posterior, effectively propagating the uncertainty on $Y_e$, $v_{\mathrm{exp}}$, and $s$ into the abundances. We also show the Solar $r$-process pattern, computed using data from \cite{lodders09} subtracted by the $s$-process residuals of \cite{bisterzo14}. The best-fit abundances are evidently non-Solar at the first two epochs, but are closer to Solar at the third. A new, redder kilonova component emerges at 3.4 days post-merger.}\label{fig:abunds_time_evolution}
\end{figure*}
...
\begin{figure}[!ht]
    ...
    \figcaption{\textbf{Leave-one-out spectra for the favoured, new models: single-component for 2.4 days, and multi-component for 3.4 days.} The preferred model for 1.4 days, not shown here, is the single-component purple+warm model of V23.}\label{fig:leave_out_pref}
\end{figure}
...
\begin{deluxetable*}{cccc}
\centering
\tablecaption{Best-fit parameters for multi-component fits to the GW170817 kilonova at $1.4$, $2.4$, and $3.4$~days. The ejecta mass in each component $M_{\mathrm{1; 2}}$, lanthanide mass fraction $X_{\mathrm{lan,1; 2}}$ in each component, and the total $X_{\mathrm{lan,total}}$ are derived parameters, \ie, they are not dimensions in $\theta$-space.}
\tablehead{parameter & 1.4 days & 2.4 days & 3.4 days}
\startdata\tablewidth{1.0\textwidth}
 \vspace{2pt}
...
\begin{figure}[!ht]
    ...
    \figcaption{\textbf{Physical setup and composition-setting parameters for the multi-component models.} Best fits are taken as the median of the posterior. From top to bottom: 1.4, 2.4, and 3.4 days.}\label{fig:infer_multicomp_wedges}
\end{figure}
...
\begin{figure*}[!ht]
    \centering
    \includegraphics[width=0.47\textwidth]{figs/appendix/221024_080947_single_all_TARDIS_evals_mactive-600_m0-1500.png}
    \includegraphics[width=0.47\textwidth]{figs/appendix/230621_082134_single_all_TARDIS_evals_mactive-600_m0-1500.png}
...
Figures \ref{fig:corner_single_2.4} and~\ref{fig:corner_single_3.4} show the full posteriors for the new single-component fits to the 2.4 and 3.4 day spectra, respectively. Samples from these single-component posteriors can be accessed online.\footnote{\redbf{Add a link to the posteriors?}}


%% corner plots
\begin{figure*}[!ht]
    \includegraphics[width=0.95\textwidth]{figs/appendix/221022_084052_dynesty_corner_pretty_smooth2d0.05_nodatapts_zoom.png}
    \figcaption{\textbf{Posterior from the single-component \SPARK~run at 2.4 days.}}\label{fig:corner_single_2.4}
\end{figure*}

\begin{figure*}[!ht]
    \includegraphics[width=0.95\textwidth]{figs/appendix/230618_143307_dynesty_corner_pretty_mactive-600_smooth2d0.05_nodatapts_zoom-midYe-lows-highdens_add-nlive-30000.png}
    \figcaption{\textbf{Same as Figure~\ref{fig:corner_single_2.4}, for the single-component \SPARK~run at 3.4 days.}}\label{fig:corner_single_3.4}
\end{figure*}
...
%     \figcaption{Posterior from the multi-component \SPARK~run at 1.4 days.}\label{fig:corner_multi_1.4}
% \end{figure*}
...
%     \figcaption{\textbf{Same as Figure~\ref{fig:corner_multi_1.4}, for the multi-component \SPARK run at 2.4 days.}}\label{fig:corner_multi_2.4}
% \end{figure*}
...
%     \figcaption{\textbf{Same as Figure~\ref{fig:corner_multi_1.4}, for the multi-component \SPARK run at 3.4 days.}}\label{fig:corner_multi_3.4}
% \end{figure*}
...
\begin{figure*}[!ht]
    ...
    \figcaption{\textbf{Leave-one-out spectra for the disfavoured models: multi-component for 1.4 and 2.4 days, and single-component for 3.4 days.}}\label{fig:leave_out_nonpref}
\end{figure*}
