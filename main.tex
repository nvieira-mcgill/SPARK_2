...
%     \item All \texttt{approxposterior}/BAPE hyperparameters and optimizers used here are the same as those used in V23. We again produce a baseline run of $m_{0} = 1500$ Latin Hypercube sampled points at the beginning of each \SPARK~run. However, the parameter space allowed by our priors differs for the 1.4, 2.4, and 3.4 day fits. Table~\ref{tab:priors-single} includes our priors for each of our single-component fits. All priors are uniform. The bounds on the density $\rho_0$ and abundance-setting parameters $Y_e$, $v_{\mathrm{exp}}$, and $s$ are the same at all epochs. The priors differ in the bounds on the luminosity, as expected given the cooling of the ejecta over time as it expands. We further allow for wider priors on the inner and outer boundary velocities for the fits at later epochs. In V23, we fixed $v_{\mathrm{outer}} = 0.35c$ during our 1.4 day fit but noted that we observed similar results for $v_{\mathrm{outer}}$ in the range $0.35 - 0.38c$. Here, we allow for greater flexibility in our fits to the later epochs. \redbf{Mention/discuss: Does this complicate the inference? Is there a degeneracy between the inner and outer boundary velocities?}
...
\startdata\tablewidth{1.0\textwidth}
 \vspace{2pt}
$\log_{10}(L_\mathrm{outer}/L_{\odot})$ & $7.782^{+0.013}_{-0.014}$ & & \\ 
$\log_{10}(\rho_0/\mathrm{g~cm^{-3}})$ & $-15.016^{+0.320}_{-0.316}$ & & \\ 
$v_{\mathrm{inner}}/c$ & $0.313^{+0.013}_{-0.014}$ & & \\
$v_{\mathrm{outer}}/c$ & $0.35$ & & \\
$v_{\mathrm{exp}}/c$ & $0.240^{+0.055}_{-0.082}$ & & \\
$Y_e$ & $0.311^{+0.013}_{-0.011}$ & & \\
$s~[k_{\mathrm{B}}/\mathrm{nucleon}]$ & $13.6^{+4.1}_{-3.0}$ & & \\ \hline
$T_{\mathrm{inner}}~[\mathrm{K}]$ & $3958^{+87}_{-94}$ & & \\ 
$X_{\mathrm{lan}}$ & $1.82^{+26.3}_{-1.79}~\times 10^{-7}$ & & \\
\enddata
...
\begin{figure*}[!ht]
    ...
    \figcaption{\textbf{All spectra in the training set, and the best fits, for the single-component fits.} \textit{Left:} Spectra corresponding to the $m_0 = 1500$ initial Latin Hypercube samples to explore parameter space and $m_{\mathrm{active}} = 600$ additional active learning points selected using BAPE, for the fit to the 2.4 day spectrum of AT2017gfo. We also show the best fit, as obtained from the median of the posteriors (see Figure~\ref{fig:corner_single_2.4}, Appendix~\ref{app:corners_single} for the full posteriors.) \textit{Right:} The same, for the 3.4 day fit. See Figure~\ref{fig:corner_single_3.4} (Appendix~\ref{app:corners_single}) for the complete posterior.}\label{fig:all_spec_single}
\end{figure*}
...
\begin{figure*}[!ht]
    ...
    \figcaption{\textbf{Best-fit abundance patterns at 1.4, 2.4 and 3.4 days, as given by the single-component fits.} Uncertainty bands are obtained by taking additional samples from the the posterior, effectively propagating the uncertainty on $Y_e$, $v_{\mathrm{exp}}$, and $s$ into the abundances. We also show the Solar $r$-process pattern, computed using data from \cite{lodders09} subtracted by the $s$-process residuals of \cite{bisterzo14}. The best-fit abundances are evidently non-Solar at each epoch. The abundances are remarkably consistent with each other, within uncertainties, indicating that we are not seeing a new kilonova component emerging, up to 3.4 days.}\label{fig:abunds_time_evolution}
\end{figure*}
...
\begin{itemize}
    ...
    \item In Figure~\ref{fig:leave_out_multi}, we show leave-one-out spectra for the best fit multi-component models. The elements which shape the spectra of our multi-component best fits are the same as those which shape our single-component: Sr, Y, and Zr. One exception is that the multi-component 2.4 day fit shows evidence for some absorption from the lanthanides, specifically lanthanum (${}_{57}$La) and neodymium (${}_{60}$Nd). There is also absorption from barium (${}_{56}$Ba), which is notably in the same periodic table group as ${}_{38}$Sr.

\end{itemize}

\begin{deluxetable}{cccc}
\centering
\tablecaption{Best-fit parameters for multi-component fits to the GW170817 kilonova at $1.4$, $2.4$, and $3.4$~days. The inner boundary temperature $T_{\mathrm{inner}}$, ejecta mass in each component $M_{\mathrm{ej}}$, and lanthanide mass fraction in each component $X_{\mathrm{lan}}$ are derived parameters, \ie, they are not dimensions in $\theta$-space.}
\tablehead{parameter & 1.4 days & 2.4 days & 3.4 days}
\startdata\tablewidth{1.0\textwidth}
 \vspace{2pt}
$\log_{10}(L_\mathrm{outer}/L_{\odot})$ &  & & \\ 
$\log_{10}(\rho_0/\mathrm{g~cm^{-3}})$ & & & \\ \hline
$v_{\mathrm{inner,red}}/c$ & & & \\
$v_{\mathrm{outer,red}}/c$ & & & \\
$v_{\mathrm{exp,red}}/c$ & & & \\
$Y_{e,\mathrm{red}}$ & & & \\
$s_{\mathrm{red}}~[k_{\mathrm{B}}/\mathrm{nucleon}]$ & & & \\ \hline
$v_{\mathrm{inner,blu}}/c$ & & & \\
$v_{\mathrm{outer,blu}}/c$ & & & \\
$v_{\mathrm{exp,blu}}/c$ & & & \\
$Y_{e,\mathrm{blu}}$ & & & \\
$s_{\mathrm{blu}}~[k_{\mathrm{B}}/\mathrm{nucleon}]$ & & & \\ \hline\hline
$T_{\mathrm{inner}}~[\mathrm{K}]$ & & & \\ \hline
$M_{\mathrm{ej,red}}~[M_{\odot}]$ & & & \\
$X_{\mathrm{lan,red}}$ & & & \\ \hline
$M_{\mathrm{ej,blu}}~[M_{\odot}]$ & & & \\
$X_{\mathrm{lan,blu}}$ & & & \\
\enddata
\end{deluxetable}\label{tab:bestfit}
...
\begin{figure*}[!ht]
    ...
    \figcaption{\textbf{All spectra in the training set, and the best fits, for the multi-component fits.} \textit{Top left:} Spectra corresponding to the $m_0 = 1500$ initial Latin Hypercube samples to explore parameter space and $m_{\mathrm{active}} = 600$ additional active learning points selected using BAPE, for the fit to the 1.4 day spectrum of AT2017gfo. We also show the best fit, as obtained from the median of the posteriors (see Figure~\ref{fig:corner_multi_1.4}, Appendix~\ref{app:corners_multi} for the full posteriors.) \textit{Top right:} The same, for the 2.4 day fit. See Figure~\ref{fig:corner_multi_2.4} (Appendix~\ref{app:corners_multi}) for the complete posterior. \textit{Bottom center:} The same for the 3.4 day fit. See Figure~\ref{fig:corner_multi_3.4} (Appendix~\ref{app:corners_multi}) for the complete posterior.}\label{fig:all_spec_multi}
\end{figure*}
...
\begin{figure}[!ht]
    ...
    \figcaption{\textbf{Physical setup and composition-setting parameters for the multi-component fits.} Best fits are taken as the median of the posterior. From top to bottom: 1.4, 2.4, and 3.4 days.}\label{fig:infer_multicomp_wedges}
\end{figure}
...
Things to think about / don't forget:

\begin{itemize}

    \item If the abundances don't really evolve, but we still see that $Y_e$ is tending to lower values over time, is that evidence for a $Y_e$ gradient/distribution (as we really expect), but across a single component?
...
In this section, we provide the full posteriors for single-component \SPARK~fits to the 2.4 and 3.4 day spectra. We refer the reader to V23 for the posterior of the single-component 1.4 day fit.
...
\begin{figure*}[!ht]
    \centering
    \includegraphics[width=0.47\textwidth]{figs/appendix/230503_184337_leaveoneout_all_TARDIS_evals_label-interest.png}
    \includegraphics[width=0.47\textwidth]{figs/appendix/230504_003423_leaveoneout_all_TARDIS_evals_label-interest.png}
